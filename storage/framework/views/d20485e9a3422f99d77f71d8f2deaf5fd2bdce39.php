<div class="card border-secondary border-top-2 border-bottom-2">
    <div class="card-header bg-light header-elements-inline">
        <h5 class="card-title">
            ...
            <b>Editar Descanso Registrado</b>
        </h5>
    </div>
    <div class="card-body">
        <form action="<?php echo e(route('recursoshumanos.descanso.update', $descanso->ID)); ?>" method="POST" role="form"
            id="formDescanso">
            <?php echo csrf_field(); ?>
                    ...
                    <select width="100%" class="form-control" required id='NOMBRE_COMPLETO' name="NOMBRE_COMPLETO">
                        ...
                        <?php $__currentLoopData = $nombrespersonas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nombrepersona): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($nombrepersona->NOMBRE.' '.$nombrepersona->APELLIDO.' |'.$nombrepersona->CARNET); ?>" 
                                <?php if($descanso->CARNET==$nombrepersona->CARNET): ?>
                                    selected
                                <?php endif; ?>
                                >
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="date" name="FECHA_SOLICITUD" id="FECHA_SOLICITUD" required maxlength="10"
                        placeholder='Fecha de Solicitud' class='form-control' value="<?php echo e($descanso->FECHA_SOLICITUD); ?>">
                </div>
            </div>
            <div class="form-group row">
                <label for="SECCION"  class="col-sm-3 offset-1 col-form-label"><b>Seccion :</b></label>
                <div class="col-sm-6">
                    <select width="100%" class="form-control" required id='SECCION' name="SECCION" maxlength="200">
                        <option></option>
                        <?php $__currentLoopData = $secciones; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $seccion): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($seccion->ID); ?>"
                                <?php if($descanso->SECCION==$seccion->ID): ?>
                                    selected
                                <?php endif; ?>    
            ...
            <div class="form-group row">
                <label for="TOTAL" class='col-3 offset-1 col-form-label'><b>Total Dias/Horas :</b></label>
                <div class="col-3">
                    <input type="text" name="TOTAL_DIAS" id="TOTAL_DIAS" readonly="true" maxlength="5"
                    placeholder='Total Dias' class='form-control' onclick="calcularDias()" onchange="calcularDias()">
                </div>
                <div class="col-3">
                    <input type="time" name="TOTAL_HORAS" id="TOTAL_HORAS" readonly="true" maxlength="8"
                    placeholder='Total Horas' class='form-control' onclick="calcularHoras()" onchange="calcularHoras()">
                </div>
            </div>
            <div class="form-group row">
                <label for="OBSERVACION" class='col-3 offset-1 col-form-label'><b>Observaciones :</b></label>
                <div class="col-6">
                    <textarea type="text" name="OBSERVACION" id="OBSERVACION" required maxlength="250"
                        placeholder='Ingrese las Observaciones' class='form-control'><?php echo e($descanso->OBSERVACION); ?></textarea>
                </div>
            </div>
            <div class="form-group row">
                <label for="FECHA_RETORNO" class='col-3 offset-1 col-form-label'><b>Fin Solicitud :</b></label>
                <div class="col-6">
                    <input type="date" name="FECHA_RETORNO" id="FECHA_RETORNO" required maxlength="10"
                        placeholder='Fecha Fin de Solicitud' class='form-control' value="<?php echo e($descanso->FECHA_RETORNO); ?>">
                </div>
            </div>
            <div class="form-group row">
...
<script>
    function  calcularDias(){
        ...
        $('#FECHA_RETORNO').val($('#FECHA_FIN_SOLICITUD').val());
    }
    ...
    function contarCaracteres(){
        $('#OBSERVACION').val();
        caracteres = $('#OBSERVACION').length;
        if (caracteres<=200) {
            console.log(caracteres);
        } else {
            alert('Esta al maximo de Caracteres a llenar en Observacion');
        }
    }
</script><?php /**PATH D:\xampp\htdocs\faboce2023\Modules/RecursosHumanos\Resources/views/descanso/edit.blade.php ENDPATH**/ ?>