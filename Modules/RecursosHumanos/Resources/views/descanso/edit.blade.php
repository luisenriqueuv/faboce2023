<div class="card border-secondary border-top-2 border-bottom-2">
    ...
    <div class="card-body">
        <form action="{{ route('recursoshumanos.descanso.update', $descanso->ID) }}" method="POST" role="form"
            id="formDescanso">
            @csrf
            @method('PUT')
                    ...
                    <select width="100%" class="form-control" required id='NOMBRE_COMPLETO' name="NOMBRE_COMPLETO">
                        ...
                        @foreach ($nombrespersonas as $nombrepersona)
                            <option value="{{ $nombrepersona->NOMBRE.' '.$nombrepersona->APELLIDO.' |'.$nombrepersona->CARNET }}" 
                                @if ($descanso->CARNET==$nombrepersona->CARNET)
                                    selected
                                @endif
                                >
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="date" name="FECHA_SOLICITUD" id="FECHA_SOLICITUD" required maxlength="10"
                        placeholder='Fecha de Solicitud' class='form-control' value="{{ $descanso->FECHA_SOLICITUD }}">
                </div>
            </div>
            <div class="form-group row">
                    ...
                    <select width="100%" class="form-control" required id='SECCION' name="SECCION" maxlength="200">
                        ...
                        @foreach ($secciones as $seccion)
                            <option value="{{ $seccion->ID }}"
                                @if ($descanso->SECCION==$seccion->ID)
                                    selected
                                @endif    
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <textarea type="text" name="OBSERVACION" id="OBSERVACION" required maxlength="250"
                        placeholder='Ingrese las Observaciones' class='form-control'>{{ $descanso->OBSERVACION }}</textarea>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="date" name="FECHA_RETORNO" id="FECHA_RETORNO" required maxlength="10"
                        placeholder='Fecha Fin de Solicitud' class='form-control' value="{{ $descanso->FECHA_RETORNO }}">
                </div>
            </div>
            <div class="form-group row">
