...
<div class="card border-secondary border-top-2 border-bottom-2">
    <div class="card-header bg-light header-elements-inline">
        <h5 class="card-title">
            ...
            <b>Codificador de Seccion</b>
        </h5>
    </div>
    <div class="card-body">
        <form action="{{ route('recursoshumanos.seccion.update', $seccion->CARNET) }}" method="POST" role="form"
            id="formSeccion">
            @csrf
            @method('PUT')
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="text" name="APELLIDO" id="APELLIDO" required maxlength="150"
                        placeholder='Ingrese el apellido' class='form-control' value="{{ $seccion->APELLIDO }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="text" name="NOMBRE" id="NOMBRE" required maxlength="150"
                        placeholder='Ingrese el nombre' class='form-control' value="{{ $seccion->NOMBRE }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <select width="100%" class="form-control" required id='IDJERARQUIA' name="IDJERARQUIA">
                        ...
                        @foreach ($jerarquias as $jerarquia)
                            @if ($jerarquia->ID == $seccion->IDJERARQUIA)
                ...
                <div class="col-sm-6">
                    <select width="100%" class="form-control" required id='AGECODIGO' name="AGECODIGO">
                        ...
                        @foreach ($fabricas as $agencia)
                            @if ($agencia->AGECODIGO == $seccion->AGECODIGO)
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="text" name="CIUDAD" id="CIUDAD" required maxlength="100"
                        placeholder='Ingrese la ciudad' class='form-control' value="{{ $seccion->CIUDAD }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-8 col-sm-4 col-md-3">
                    <input type="date" name="FECHAI" id="FECHAI" required value="{{ date('Y-m-d') }}"
                        class='form-control text-right' value="{{ $seccion->FECHAI }}">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <select width="100%" class="form-control" required id='IDFORMULARIO' name="IDFORMULARIO">
                        ...
                        @foreach ($formularios as $formulario)
                            @if ($formulario->ID == $seccion->IDFORMULARIO)
                        ...
                        @foreach ($fabricas as $agencia)
                            @if (
                                $agencia->AGECODIGO > 0 &&
                                    ...
                                    $agencia->AGECODIGO == $seccion->AGECODIGO2)
                                <option value="{{ $agencia->AGECODIGO }}" selected>{{ $agencia->AGECODIGO }} -
                                    {{ $agencia->AGENOMBRE }}</option>
                            @else
                                <option value="{{ $agencia->AGECODIGO }}">{{ $agencia->AGECODIGO }} -
