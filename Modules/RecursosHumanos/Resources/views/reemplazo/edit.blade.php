<div class="card border-secondary border-top-2 border-bottom-2">
    ...
    <div class="card-body">
        <form action="{{ route('recursoshumanos.reemplazo.update', $reemplazo->ID) }}" method="POST" role="form"
            id="formReemplazo">
            @csrf
            @method('PUT')
                    ...
                    <select width="100%" class="form-control" required id='NOMBRE_COMPLETO' name="NOMBRE_COMPLETO">
                        ...
                        @foreach ($nombrespersonas as $nombrepersona)
                            <option value="{{ $nombrepersona->NOMBRE.' '.$nombrepersona->APELLIDO.' |'.$nombrepersona->CARNET }}" 
                                @if ($reemplazo->CARNET==$nombrepersona->CARNET)
                                    selected
                                @endif
                                >
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    <input type="date" name="FECHA_SOLICITUD" id="FECHA_SOLICITUD" required maxlength="10"
                        placeholder='Fecha de Solicitud' class='form-control' value="{{ $reemplazo->FECHA_SOLICITUD }}">
                </div>
            </div>            
            <div class="form-group row">
                    ...
                    <select width="100%" class="form-control" required id='SECCION' name="SECCION">
                        ...
                        @foreach ($secciones as $seccion)
                            <option value="{{ $seccion->ID }}"
                                @if ($reemplazo->SECCION==$seccion->ID)
                                    selected
                                @endif    
            ...
            <div class="form-group row">
                ...
                <div class="col-3">
                    <input type="date" name="FECHA_INICIO_SOLICITUD" id="FECHA_INICIO_SOLICITUD" required maxlength="10"
                        placeholder='Fecha de Solicitud' class='form-control' value="{{ $reemplazo->FECHA_INICIO_SOLICITUD }}" onclick="calcularDias()" onchange="calcularDias()">
                </div>
                <div class="col-3">
                    <input type="time" name="HORA_INICIO_SOLICITUD" id="HORA_INICIO_SOLICITUD" maxlength="8"
                    placeholder='Hora de Solicitud' class='form-control' value="{{ $reemplazo->HORA_INICIO_SOLICITUD }}" onclick="calcularHoras()" onchange="calcularHoras()">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-3">
                    <input type="date" name="FECHA_FIN_SOLICITUD" id="FECHA_FIN_SOLICITUD" required maxlength="10"
                        placeholder='Fecha Fin de Solicitud' class='form-control' value="{{ $reemplazo->FECHA_FIN_SOLICITUD }}" onclick="calcularDias()" onchange="calcularDias()">
                </div>
                <div class="col-3">
                    <input type="time" name="HORA_FIN_SOLICITUD" id="HORA_FIN_SOLICITUD" maxlength="8"
                    placeholder='Hora Fin de Solicitud' class='form-control' value="{{ $reemplazo->HORA_FIN_SOLICITUD }}" onclick="calcularHoras()" onchange="calcularHoras()">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-3">
                    <input type="text" name="TOTAL_DIAS" id="TOTAL_DIAS" maxlength="10"
                        placeholder='Total Dias' class='form-control' readonly value="{{ $reemplazo->TOTAL_DIAS }}" onclick="calcularDias()" onchange="calcularDias()">
                </div>
                <div class="col-3">
                    <input type="text" name="TOTAL_HORAS" id="TOTAL_HORAS" maxlength="8"
                        placeholder='Total Horas' class='form-control' readonly value="{{ $reemplazo->TOTAL_HORAS }}" onclick="calcularHoras()" onchange="calcularHoras()">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-6">
                    <textarea type="text" name="OBSERVACION" id="OBSERVACION" required maxlength="255"
                        placeholder='Ingrese las Observaciones' class='form-control'> {{ $reemplazo->OBSERVACION }}</textarea>
                </div>
            </div>
            <div class="form-group row">
