...
class TiporegistroRepository
{
    private $_model;
    private $_db;

    public function __construct(Tiporegistro $model)
    {
        $this->_model = $model;
    ...
    {
        return $withTrashed ?
            $this->_model->on($this->_db)->withTrashed()->get(['ID', 'TITLE']) :
            $this->_model->on($this->_db)->get(['ID', 'TITLE']);
    }
