...
class CambioturnoRepository
{
    private $_model;
    private $_db;

    public function __construct(Cambioturno $model)
    {
        $this->_model = $model;
    ...
    public function all()
    {
        return $this->_model->on($this->_db)
            ->withTrashed()
            ->orderBy('ID', 'ASC')
            ->get(['ID','CARNET','NOMBRE_COMPLETO','FECHA_SOLICITUD','SECCION','FECHA_INICIO_SOLICITUD','FECHA_FIN_SOLICITUD','NOMBRE_REEMPLAZO','TURNO_REEMPLAZO','NOMBRE_REEMPLAZADO','TURNO_REEMPLAZADO','DELETED_AT','DELETED_ID']);
    }
    ...
    {
        return $this->_model->withTrashed()->with('cambioturno')->find($id);
    }
    ...
    {
        return $this->_model->on($this->_db)->find($id, ['ID','CARNET','NOMBRE_COMPLETO','FECHA_SOLICITUD','SECCION','FECHA_INICIO_SOLICITUD','FECHA_FIN_SOLICITUD','NOMBRE_REEMPLAZO','TURNO_REEMPLAZO','NOMBRE_REEMPLAZADO','TURNO_REEMPLAZADO','DELETED_AT','DELETED_ID','ID']);
    }
